...
trait CommonHelper
{
     ...
     * @return array|string|string[]
     */
    function replaceInvalidFileNameCharacters($txt) {
        $invalidCharacters = ['\\', '/', ':', '*', '?', '"', '<', '>', '|', '(', ')'];
        return str_replace($invalidCharacters, '_', $txt);
    }
    ...
    public function buildCronExpression($amount, $occurrence, $time)
    {
        ...
        if (!in_array($occurrence, ['DAY', 'WEEK', 'MONTH', 'YEAR'])) {
            throw new \InvalidArgumentException('Invalid occurrence value.');
        }
        ...
        if ($occurrence === 'DAY') {
            // Runs every $amount day/s at $hour:$minutes
            return "{$minutes} {$hour} */{$amount} * *";
        } elseif ($occurrence === 'WEEK') {
            $days = "*/" . ($amount * 7);
            // Runs every $amount week/s at $hour:$minutes
            return "{$minutes} {$hour} * * {$days}";
        } elseif ($occurrence === 'MONTH') {
            // Runs every $amount month/s at $hour:$minutes
            return "{$minutes} {$hour} 1 */{$amount} *";
        } elseif ($occurrence === 'YEAR') {
            // Runs every $amount year/s at $hour:$minutes
            return "{$minutes} {$hour} 1 1 */{$amount}";
        }
    }
}
